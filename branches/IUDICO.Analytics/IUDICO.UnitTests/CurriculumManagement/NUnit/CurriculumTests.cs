...
namespace IUDICO.UnitTests.CurriculumManagement.NUnit
{
    ...
    public class DisciplineTests
    {
        ...
        public void AddChapter()
        {
            ...
            ids.Select((item, i) => i).ToList().ForEach(
                item => AdvAssert.AreEqual(chapters[item], _Storage.GetChapter(ids[item])));
            try
            {
        ...
        public void GetTopic()
        {
            ...
            var chapters = disciplines.Select(item => new Chapter {Name = "Chapter", Discipline = item}).ToList();
            var topics =
                chapters.Select(item => new Topic {Name = "Topic", Chapter = item, TopicType = _Storage.GetTopicType(1)})
                    .ToList();
