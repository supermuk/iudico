// --------------------------------------------------------------------------------------------------------------------
// <copyright company="" file="ContentHelper.cs">
//   
// </copyright>
// 
// --------------------------------------------------------------------------------------------------------------------
...
namespace Microsoft.LearningComponents.Frameset
{
    ...
    /// </summary>
    public delegate bool ProcessPostedData(
    ...
    /// </summary>
    public delegate void UpdateRenderContext(
        RenderContext renderContext, StringBuilder onLoadScript, LearningSession session);
    ...
    {
        private UpdateRenderContext mUpdateRenderContext;

        private UpdateRenderContext UpdateRenderContext
        {
            get
            {
                return this.mUpdateRenderContext;
            }
            set
            {
                this.mUpdateRenderContext = value;
            }
        }

        private bool mIsPostedPage; // if true, the page was posted
        ...
        // delegate parameters that are called as methods are cased as methods
        ...
        public void WriteFrameMgrInit()
        {
            ...
            if (this.HasError || this.SubmitPageDisplayed)
            {
                ...
                this.Response.Write("frameMgr.SetContentFrameUrl(null); \r\n");
            }
            ...
            {
                activityId = (this.Session.HasCurrentActivity
            ...
            if (this.SubmitPageDisplayed)
            {
                ...
                this.Response.Write("frameMgr.WaitForContentCompleted(0);\r\n");
            }
            else
            {
                this.Response.Write(
            ...
            if (this.mIsPostedPage)
            {
                ...
                this.Response.Write("frameMgr.PostIsComplete();");
            }
        }
        ...
        private bool IsPrimaryResource(string resPath)
        {
            ...
            Uri resXmlBase = this.Session.CurrentActivityResourceXmlBase;
            ...
            // return true.
            if (Uri.Compare(
                this.Session.CurrentActivityEntryPoint,
                resourcePath,
                UriComponents.Path,
                UriFormat.SafeUnescaped,
                StringComparison.OrdinalIgnoreCase) == 0)
        ...
        private bool IsPrimaryResourceOfLrmActivity(string resPath, out long pageId)
        {
            ...
            if (this.Session.PackageFormat != PackageFormat.Lrm)
            {
                return false;
            }
            ...
            // use the local variable to make fxcop happy
            if (isRelative == null)
            {
                return false;
            }
            ...
            string[] simplePathSegments = simplePath.Split(new[] { '/' });
        ...
        private static string GetSimplePath(string path)
        {
            ...
            string[] retValSegments = retVal.Split(new[] { '/' });
        ...
        // catching all exceptions in order to return http response code
        private void RenderPackageContent(string resPath)
        {
            ...
            if (this.IsPrimaryResource(resPath))
            {
                ...
                if (this.Session.CurrentActivityResourceType == ResourceType.None)
                {
                    this.RegisterError(
                        ResHelper.GetMessage(Localization.GetMessage("CON_ResourceNotFoundTitle")),
                        ResHelper.GetMessage(Localization.GetMessage("CON_ResourceNotFoundMsg")),
                        false);
                    return;
                }
                ...
                if (this.Session.CurrentActivityEntryPoint.IsAbsoluteUri)
                {
                    this.RegisterError(
                        ResHelper.GetMessage(Localization.GetMessage("CON_ResourceNotFoundTitle")),
                        ResHelper.GetMessage(Localization.GetMessage("CON_ResourceNotFoundMsg")),
                ...
                try
                {
                    string filePath = this.Session.CurrentActivityEntryPoint.OriginalString.Replace("%20", " ");
                    RenderContext context = new RenderContext(filePath, this.Response);

                    StringBuilder scriptBlock = new StringBuilder(1000);

                    // Initialize the hidden control and script for the page
                    this.InitHiddenControlInfo(context, scriptBlock);

                    // Allow application to modify the context. 
                    this.UpdateRenderContext(context, scriptBlock, this.Session);

                    this.FinishOnLoadScript(scriptBlock);
            ...
            {
                long pageId;
                if (this.IsPrimaryResourceOfLrmActivity(resPath, out pageId))
                {
                    ...
                    this.Session.MoveToActivity(activityId);
                    ...
                    this.Response.Clear();
                    this.Response.Redirect(this.GetRedirectLocation());
                }
                else
                {
                    ...
                    try
                    {
                        RenderContext context = new RenderContext(resPath.Replace("%20", " "), this.Response);
                        this.UpdateRenderContext(context, null, this.Session);
                        this.Session.Render(context);
        ...
        private void InitHiddenControlInfo(RenderContext context, StringBuilder onLoadScript)
        {
            ...
            if (this.Session.CurrentActivityResourceType != ResourceType.Lrm)
            {
                return;
            }
            ...
            {
                controls.Add(
                    HiddenFieldNames.AttemptId, FramesetUtil.GetStringInvariant(this.Session.AttemptId.GetKey()));
                onLoadScript.AppendFormat(
            ...
            controls.Add(HiddenFieldNames.View, FramesetUtil.GetString(this.Session.View));
            onLoadScript.AppendFormat(
            ...
            // Write the current activity Id. Write -1 if there isn't one.
            var currentActivityId = this.Session.HasCurrentActivity
                                        ? FramesetUtil.GetStringInvariant(this.Session.CurrentActivityId)
                                        : "-1";
            controls.Add(
                HiddenFieldNames.ActivityId,
                currentActivityId);
            onLoadScript.AppendFormat(
                "frameMgr.SetActivityId(document.getElementById({0}).value);\r\n",
        ...
        private static void WritePleaseWaitToResponse(HttpResponse response)
        {
            ...
            response.Write(ResHelper.GetMessage(Localization.GetMessage("CON_PleaseWait")));
