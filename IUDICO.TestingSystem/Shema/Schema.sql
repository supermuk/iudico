...
SET @schema = @schema +
    ...
    '</ItemType>'
SET @schema = @schema +
    '<ItemType Name="PackageItem" ViewFunction="PackageItem$DefaultView" UpdateSecurityFunction="PackageItem$UpdateSecurity">' + 
...
SET @schema = @schema +
    ...
    '</ItemType>'
SET @schema = @schema +
    '<ItemType Name="UserItem" ViewFunction="UserItem$DefaultView" AddSecurityFunction="UserItem$AddSecurity">' + 
        '<Property Name="Key" TypeCode="2" Nullable="false" HasDefault="false"/>' +
...
SET @schema = @schema +
    ...
    '</View>'
SET @schema = @schema +
    '<View Name="Me" Function="Me" SecurityFunction="Me$Security">' + 
        '<Column Name="UserId" TypeCode="1" Nullable="true" ReferencedItemTypeName="UserItem"/>' +
        '<Column Name="UserName" TypeCode="2" Nullable="true"/>' +
    '</View>'
SET @schema = @schema +
    '<View Name="PackageIdByCourse" Function="PackageIdByCourse" SecurityFunction="PackageIdByCourse$Security">' + 
        '<Column Name="PackageId" TypeCode="1" Nullable="true" ReferencedItemTypeName="PackageItem"/>' +
...
SET @schema = @schema +
    '<View Name="SequencingLog" Function="SequencingLog" SecurityFunction="SequencingLog$Security">' + 
        ...
        '<Column Name="Timestamp" TypeCode="4" Nullable="true"/>' +
        '<Column Name="EventType" TypeCode="8" Nullable="true" EnumName="SequencingEventType"/>' +
        '<Column Name="NavigationCommand" TypeCode="8" Nullable="true" EnumName="NavigationCommand"/>' +
        '<Column Name="Message" TypeCode="2" Nullable="true"/>' +
...
SET @schema = @schema +
    ...
    '</Right>'
SET @schema = @schema +
    '<Right Name="CreateAttemptRight" SecurityFunction="CreateAttemptRight">' + 
    ...
    '</Right>'
SET @schema = @schema +
    '<Right Name="DeleteAttemptRight" SecurityFunction="DeleteAttemptRight">' + 
        '<Parameter Name="AttemptId" TypeCode="1" Nullable="true" ReferencedItemTypeName="AttemptItem"/>' +
    '</Right>'
SET @schema = @schema +
...
CREATE FUNCTION [UserItem$AddSecurity](@UserKey nvarchar(250),@Key nvarchar(250),@Name nvarchar(255),@AudioCaptioning int=0,@AudioLevel float(24)=1,@DeliverySpeed float(24)=1,@Language nvarchar(255)='')
RETURNS bit
AS
BEGIN
...
END
GO
GRANT EXECUTE ON [UserItem$AddSecurity] TO LearningStore
GO
...
CREATE FUNCTION [Me](@UserKey nvarchar(250))
RETURNS TABLE
AS
RETURN (
    SELECT Id AS UserId,
...
GO
GRANT SELECT ON [Me] TO LearningStore
GO

-- Create function for the security on the Me view
CREATE FUNCTION [Me$Security](@UserKey nvarchar(250))
RETURNS bit
AS
BEGIN
    RETURN (1)
END
GO
GRANT EXECUTE ON [Me$Security] TO LearningStore
GO

-- Create a function that implements the PackageIdByCourse view
CREATE FUNCTION [PackageIdByCourse](@UserKey nvarchar(250),@IudicoCourseRef int=NULL)
RETURNS TABLE
AS
RETURN (
...
GO
GRANT SELECT ON [PackageIdByCourse] TO LearningStore
GO

-- Create function for the security on the PackageIdByCourse view
CREATE FUNCTION [PackageIdByCourse$Security](@UserKey nvarchar(250),@IudicoCourseRef int=NULL)
RETURNS bit
AS
BEGIN
    RETURN (1)
...
CREATE FUNCTION [SequencingLog](@UserKey nvarchar(250),@AttemptId bigint=NULL)
RETURNS TABLE
AS
RETURN (
    SELECT Id,
...
GO
GRANT SELECT ON [SequencingLog] TO LearningStore
GO

-- Create function for the security on the SequencingLog view
CREATE FUNCTION [SequencingLog$Security](@UserKey nvarchar(250),@AttemptId bigint=NULL)
RETURNS bit
AS
BEGIN
...
END
GO
GRANT EXECUTE ON [SequencingLog$Security] TO LearningStore
GO

-- Create function for the security on the AddPackageReferenceRight right
CREATE FUNCTION [AddPackageReferenceRight](@UserKey nvarchar(250))
RETURNS bit
AS
BEGIN
    RETURN (1)
...
CREATE FUNCTION [DeleteAttemptRight](@UserKey nvarchar(250),@AttemptId bigint=NULL)
RETURNS bit
AS
BEGIN
    RETURN (CASE WHEN EXISTS(SELECT * FROM AttemptItem INNER JOIN UserItem ON AttemptItem.LearnerId=UserItem.Id WHERE AttemptItem.Id=@AttemptId AND UserItem.[Key]=@UserKey) THEN 1 ELSE 0 END)
END
GO
GRANT EXECUTE ON [DeleteAttemptRight] TO LearningStore
GO
